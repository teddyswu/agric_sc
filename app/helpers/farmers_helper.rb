module FarmersHelper
	...
	def render_mobile_tag(url)
		if url.downcase.include?("jp") 
			...
			return "<video id=\"movie\" preload controls loop width=\"100%\">
        <source src=\"#{url}\" type=\"video/mp4\" />
      </video>".html_safe
		end
	end
	...
	def render_is_mobile_link(boo, id, record_id, i)
		if boo == true
			"<a class=\"position-relative d-block btn-info\" style=\"background-color:#fff\" href=\"#{farm_mobile_img_path(id, record_id)}\">".html_safe
		else
