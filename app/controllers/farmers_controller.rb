class FarmersController < ApplicationController
	...
	def show
		...
		@work_records = WorkRecord.where(:id => record_ids, :owner_id => params[:id]).order(work_time: :desc).paginate(:page => params[:page], per_page: 6)
		@work_record_all = WorkRecord.where(:owner_id => params[:id]).order(work_time: :desc)
		render layout: "story"
	end
